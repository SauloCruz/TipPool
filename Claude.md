...
| Tavern Law day screen is ONE page (2026-08-30) | The four-step wizard (Confirm / Enter / Review / Lock) is retired for TL: Square now pulls six of the seven daily figures, so the walk-through was mostly clicking "next" past values that were already right. Everything is on one scrolling screen in the Poquitos order — money, hours, event, distribution, what-gets-locked — with a single Finalize button. **The wizard's per-step gates survive as blocks on that button**, which is the only irreversible action: unmapped Square fields and unresolved missing clock-outs disable it, and a suspicious all-$0 cash night takes two presses ("Confirm $0 cash tips, then finalize"), so the manager still says yes explicitly. Two cuts to scroll: the event block collapses when the night had no event money AND no unattached deposit (a waiting deposit forces it open — hidden money is money nobody pays out), and roster members with no hours are behind a "+N more" toggle, keeping anyone with hours, a pending clock-out or a door shift visible. La Fontana keeps its stepper for now. Supersedes the 2026-07-07 "the stepper is the only day screen" ruling for TL only. |
| Contract labour (2026-08-30) | Someone who works shifts and shares the tip pool but has **no Square account** and is paid directly against a W-9 (Tavern Law uses it "from time to time" — Ray and Angelica live only in the spreadsheet, which is why their hours were never captured). Schema **v11**: `employee.is_contractor`, `hourly_rate_cents`, `w9_received`. Pay = **hours x rate + their pool share**; the $600 reporting threshold is counted **per venue, per calendar year**, from FINALIZED days only. A rate is required (without one the threshold undercounts silently) and a contractor may not also hold a Square account — the two states are exclusive, and mixing them puts the same shifts on a 1099 and a W-2. Setting the flag forces `in_payroll` off, and contractors are dropped from the payroll export AND its CSV: a 1099 worker imported into payroll gets **paid twice**, once by the import and once by hand. Their hours live in their own `contractor_hours` input, NOT `foh_hours` — typing into the pulled map would mark it a manager override and freeze every other person's hours against the next pull. They are always visible on the day screen (no timecard means 0 h forever, and hiding them is how a night goes unpaid). **The pool side decides where the share comes from** (owner 2026-08-30): an FOH contractor's typed hours weigh the FOH pool like anyone else's — a host shift takes the Door toggle and its half credit — while a KITCHEN contractor shares the even per-head split and their hours never enter it, so they are recorded for pay only and must NOT reach `foh_hours` or every FOH share is diluted by someone who was never in that pool. Kitchen contractors get an hours field on the roster chip; FOH ones use the ordinary hours row. **All three venues (2026-08-30):** each keeps contractor input in its own field so the pulled one is never marked an override — `contractor_hours` at Tavern Law and La Fontana, `contractor_shifts` (with a hand-picked role) at Poquitos, where the role normally comes off the Square job chosen at clock-in and a contractor never clocks in. LF does not track hours at all (single shift, even split) and stores a 1 h worked marker, which would have priced a whole night at one hour — contractors there get a real hours field that still does not change anyone's share. Moving someone onto payroll later = link their Square account and clear the flag; the same employee record keeps every night worked. |
| Gratuity has more than one name (2026-08-30) | `gratuity_service_charge.name_contains` now takes a **list** (or a comma-separated string), because one venue rings gratuity two ways: Tavern Law's 2026-08-05 charges were CUSTOM ones literally named **"Service Charge"**, matching neither the `AUTO_GRATUITY` type nor "gratuity", so **$59.80 vanished from the day** while the spreadsheet paid it out. And a service charge that matches nothing is now REPORTED on the day (`unmatched_service_charge`) instead of being dropped in silence — a recognised house charge stays quiet, anything else asks for a human. Setting the venue's list to "gratuity, service charge" closed the whole 2026-08-01..15 gratuity gap. |
| Hours round ONCE per person per day (2026-08-30) | The 0.05 round-up is applied to the day's total tippable seconds, not to each timecard. Rounding per punch paid a split shift more than an unbroken one: Jacob Ruley's 2026-08-13 was 17:00-22:46 then 22:46-00:36 — exactly 7.00 tippable hours — but read as 5.80 + 1.25 = **7.05**. Shift rows on the day screen now show their own UNROUNDED clipped hours, so they need not sum to the credited total. Does not change the ruling, only where it is applied. |
| A one-minute kitchen punch is flagged (2026-08-30) | "BOH worked = any kitchen timecard that day" means a stray punch takes a full even share: Jose Medina clocked 17:00-17:01 on 2026-08-01 and 23:31-23:32 on 08-13, and the app split the allocation four ways where the spreadsheet split it three. The roster still counts him — only a manager may take someone off it — but a kitchen punch under `min_shift_minutes` (default 15) now raises `short_kitchen_shift` naming the person and the length. Warning only: it never moves money by itself. |
| Aug 1-15 reconciliation (2026-08-30) | Full-period test against the live spreadsheet. Food sales and cash tips matched on **all 15 days**; credit tips on 14 (8/07 differs only because the event ticket's money moved to the event line). With the three corrections a manager would make on the day screen — door marks for Tyler/Padraig, Bree's un-clocked 8/09 shift, Jose's mis-punches — **the grand total agrees to $0.03 on $17,431.68** and every FOH row is within $11 except Jacob (+$32.20, a second punch the sheet missed). The residual differences are all cases where the app is right and the sheet was not. |
| Date navigation on every day screen (2026-08-30) | All three day screens now carry the same three affordances: **‹ ›** to step a day, a **🗓 button**, and the date itself tappable (a transparent `input[type=date]` overlaying the heading). Tavern Law had only the invisible overlay and La Fontana only the button — each alone is undiscoverable, and La Fontana's input was `width:0;pointer-events:none` so it could not be tapped at all. One shared `datePickButton()` helper, asserted used exactly three times, so they cannot drift apart again. |
| Payroll features are venue-agnostic (2026-08-30) | **Payroll entry sheet, take-home stubs, the timecard backfill and the paid-hours tile now run at every venue** — a timecard is a timecard whatever the tip policy is. Required carrying `start_at`/`end_at`/`rate_cents` on TL and LF timecards too (they had hours only), teaching `labor_hours_for` to read `raw.timecards` as well as `raw.shifts`, and dispatching `refresh_labor_shifts` per model. The payroll TIPS line is one shared `payroll_tips()` helper summing whatever that model pays as tips — points-model event money, the hourly pool's kitchen share — with auto-gratuity deliberately excluded because it is wages. **Contractors are filtered out of `payroll` at every venue** (1099 staff imported into payroll get paid twice). Verified on live data: TL 170.92 h and LF 470.27 h appear after a backfill with **every finalized payout byte-identical**. **Form 4070 stays LF-monthly-only** — it is a tip-out venue's IRS report, genuinely policy-specific. Days pulled before this change read "needs re-pull" until the backfill runs. |
| A save must round-trip what it does not show (2026-09-01) | The Poquitos day screen's `collectInputs()` omitted `net_sales_cents`, so **any edit to a day silently zeroed it**; the zero then differed from the stored pull, `merge_pull_into_inputs` read that as a manager override, and **no amount of re-pulling could restore it** — 2026-08-29 sat at $0 with $14,673.10 in the pull, permanently suppressing the period's average tip rate. Two fixes: the screen now round-trips the field (Tavern Law already did this for `foh_role_weights`), and `net_sales_cents` joins **`DERIVED_FIELDS`** so a pull always wins — correct because it is reporting-only and no human ever types it, which also repairs already-stranded days on the next pull. A test now asserts every screen round-trips every field its model pulls. |
| The payroll sheet explains its own total (2026-09-01) | Contract labour is paid directly and is deliberately absent from the payroll sheet, which makes the sheet's totals **short of the venue's own reports by the contractors' share** — Tavern Law 2026-08-16..31 read $630.18 of auto-gratuity on the period tile (matching Square exactly) but $621.66 on the payroll sheet, the $8.52 being one contractor, with nothing on the page to say why. `totals.contractor_pay` now carries the names and the tips/gratuity held back, and the sheet footnotes it. Same principle as `off_payroll_with_pay`: money kept off a report is named, never silently dropped. |
| Event gratuity is wages, not event tips (2026-09-01) | **Supersedes the 2026-08-29 ruling** that rolled an event ticket's gratuity service charge into Event Tips. A service charge is **wages wherever it was rung**, so it stays on the auto-gratuity line — right tax treatment, and it keeps that line reconcilable against the point-of-sale's own service-charge report (TL 8/16–8/31 read $574.38 against Square's $630.18; the $55.80 gap was 8/22's event charge). `extract_event_tips` now counts only a voluntary **card tip** on an event ticket and merely REPORTS the charge; `extract_auto_gratuity` no longer excludes event orders. Event card tips still move to the event. Found because the charge was landing in neither line — held out of auto-gratuity but blocked from event tips by the hand-typed deposit, so $55.80 was paid to nobody. |
| Paid hours exclude unpaid breaks (2026-09-16) | Paid hours, overtime and wages now subtract **unpaid breaks**, as Square's labor report and the tip pool already did. Every venue's stored timecard carries `unpaid_breaks` (closed, unpaid only) and `split_at_midnight` removes each break from whichever calendar-day piece it overlaps. Found at Poquitos 2026-09-01..15: Camilo Posse read **21.82 h against Square's 18.05** — 1.80 h and 1.97 h unpaid breaks — and Cleto Rojas carried four 30-minute breaks; August reconciled exactly only because nobody took a break that fortnight. **A shift stored without the key is treated as unknown** ("needs re-pull") rather than trusted, since it cannot say whether it had a break — one press of Fetch timecard data per period fixes it, and cannot move a payout. Tips and gratuity were never affected. |
| Overtime lookback days are fetched and reported (2026-09-16) | A period starting mid-week needs the previous period's days to settle the first week's overtime. Two gaps, both surfaced when the break fix made pre-fix shifts untrustworthy: the labor reader **silently dropped** a lookback day it could not trust (only in-period days were named as "needs re-pull"), and **Fetch timecard data only fetched the period itself**. Poquitos 2026-09-01..15 (a Tuesday start) therefore reported **2.58 h overtime against Square's 5.03** — Sun 8/30 and Mon 8/31 were missing from the first week. Now any untrusted day in the loaded range, lookback included, is named in `hours_unknown_dates`, and `refresh-labor` reaches back to the start of the workweek containing the period start. After one fetch the period reconciles exactly: paid 1819.17 / regular 1814.13 / overtime 5.03, no payout moved. |
| Paid hours follow the Square account's reporting day (2026-09-16) | Paid hours and overtime split a shift at the **point-of-sale account's reporting-day start**, not always midnight — setting `labor_day_start_minutes` (0–360, Setup card "Paid hours: reporting day"), independent of the tip pool's `day_cutoff_minutes`. **Tavern Law = 180 (Square reports 3:00 am–2:59 am); Poquitos = 0**, where the Aug 1–15 run reconciled at midnight. Found via Bree Staab, TL 9/1–9/15: her 8/31 15:59–01:02 shift put 1.03 h into the new period — app 28.70 h vs Square Payroll 27.67. At 180 she reads 27.67; Collin Finley, Jacob Ruley and Talia Gragg lose their 8/31 closing hour, and Kelly Lemons, Jarod Schiebout, Talia and Rachel Smith gain their 9/15 one. The reader loads one day past the period end so a 2:30 am punch stored on the next business day is still counted. Inferred from one confirmed shift plus the reporting-day label on TL's own sales report — spot-check another name before relying on it. |
| Kitchen and manager punches keep their clock (2026-09-16) | Tavern Law stored kitchen roster punches and EXCLUDED (manager) shifts **without clock times**, since neither earns an hourly tip share — so the payroll sheet silently omitted every kitchen and manager hour (TL 9/1–9/15: 699.40 h reported, 995.48 h actually paid, 13.62 h of kitchen overtime invisible). Both branches now store `start_at`/`end_at`/`unpaid_breaks`/`rate_cents` via `_clock()`. The labor reader now also refuses **partial** days — any closed timecard stored without the break field makes the day "needs re-pull" — instead of dropping that one card while the rest of the day looks complete. |
