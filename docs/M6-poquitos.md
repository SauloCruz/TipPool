...
| Card processing fee (2026-08-13, **base widened 2026-08-14**) | Withheld before pooling, so every share is of money the venue actually received. The base is **everything the card processor handled — credit tips AND auto-gratuity**; each pool bears its own share, rounded once to the cent, and the gratuity is distributed net. **Only CASH is exempt** — no processor touches it. (The original 2026-08-13 rule was credit tips only; widening it closed the last gap with the venue's previous tip-pool service and made 2026-08-05 reconcile at $1,052.42 exactly.) Setup input `poq_card_fee_pct`, default `0` (never assumed); the rate used is recorded on each snapshot. |
